	...
	if (tx.vin[0].hasOwnProperty('coinbase')) {
		...
		for (let v of tx.vin) {
			...
			if (vin === null) {
				...
				console.log('stopped at txid:', tx.txid);
				...
				console.log('Invalid block height:', block.height);
				...
				if (invBlock.confirmations === 0 && (block.confirmations > invBlock.confirmations)) {
					...
					for (let dbTx of dbBlock.tx) {
						...
						for (let invTx of invAddrTxs) {
							...
							const addressId = invAddress._id;

							const resAddress = await addr.replaceOne({ _id: addressId }, invAddress);
							console.log('Address: ' + resAddress.ops[0]._id + ' updated  ');
						}
...
module.exports = { prepareTxs, prepareBlock, getInputAddresses, getRecipients, prepareVins, prepareVouts, getAllPagesFetched, setAmountsOut, getPageData, /*getCalculatedConfirmations,*/ isPositiveInteger, isValidHash, isValidAddress, getPeers };